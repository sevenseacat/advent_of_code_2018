defmodule Day23 do
  ...
  def part2(bots) do
    ...
    {x, y, z} = do_part2(bots, {min_x, max_x}, {min_y, max_y}, {min_z, max_z}, 10000000)
    x + y + z
  end
  ...
  defp do_part2(bots, {min_x, max_x}, {min_y, max_y}, {min_z, max_z}, factor) do
    {{x, y, z}, _, _} = Enum.reduce(div(min_x, factor)..div(max_x, factor), {nil, 0, 0}, fn x, x_acc ->
      Enum.reduce(div(min_y, factor)..div(max_y, factor), x_acc, fn y, y_acc ->
        Enum.reduce(div(min_z, factor)..div(max_z, factor), y_acc, fn z, ({_coord, distance, bots_in_range} = acc) ->
    ...
    do_part2(bots, {x*factor, (x+1)*factor}, {y*factor, (y+1)*factor}, {z*factor, (z+1)*factor}, div(factor, 10))
  end

  defp count_bots_in_range({x1, y1, z1}, bots, f) do
    Enum.count(bots, fn %{x: x2, y: y2, z: z2, r: r} ->
      within_radius?(%{x: div(x2, f), y: div(y2, f), z: div(z2, f), r: div(r, f)}, %{x: x1, y: y1, z: z1})
    end)
  end
  ...
  def bench do
    Benchee.run(
      %{
        "day 23, part 1" => fn -> Advent.data(23) |> parse_input |> part1 end,
        "day 23, part 2" => fn -> Advent.data(23) |> parse_input |> part2 end
      },
